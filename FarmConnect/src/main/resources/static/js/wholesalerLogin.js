...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // OAuth Login Role Handling
    const oauthLoginButton = document.getElementById("oauthLoginButton");
    if (oauthLoginButton) {
        oauthLoginButton.addEventListener("click", (event) => {
            event.preventDefault();
            fetch('/store-role', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ role: "ROLE_WHOLESALER" })
            })
            .then(() => { window.location.href = "/oauth2/authorization/google"; })
            .catch(error => console.error("Error storing role:", error));
        });
    }
});
